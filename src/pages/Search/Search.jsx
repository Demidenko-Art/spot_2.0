    ...
    <div className="search-page">
      ...
      {query ? (
        <div className="search-results-wrapper">
          <p className="s-count">Знайдено: <span>{filtered.length}</span></p>
          {filtered.length === 0 ? (
            <div className="search-empty-state">
              <span className="se-icon">🔍</span>
              <p>Нічого не знайдено</p>
              <span className="se-sub">Перевірте правильність написання назви</span>
            </div>
          ) : (
        ...
        <div className="search-placeholder-state">
          ...
          <span>Введіть перші літери назви альбому, треку чи імені виконавця.</span>
        </div>
      )}
    </div>
  );
